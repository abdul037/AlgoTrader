...
def test_metrics_exposes_reconciliation_trading_and_safety_gauges(tmp_path):
    ...
    assert "algobot_strategy_families_total 24" in response.text
    assert "algobot_strategy_specs_total 62" in response.text
    assert "algobot_strategy_families_enhanced_total 12" in response.text
    assert "algobot_strategy_specs_enhanced_total 26" in response.text
