...
class ScreenerFilterPipeline:
    ...
    def evaluate(
        ...
        *,
        ...
        accuracy_score = float(signal.metadata.get("accuracy_score") or 0.0)
        confirmation_score = float(signal.metadata.get("confirmation_score") or 0.0)
        false_positive_risk = float(signal.metadata.get("false_positive_risk_score") or 1.0)
        ...
        accuracy_floor = float(self.settings.screener_min_accuracy_score)
        confirmation_floor = float(self.settings.screener_min_confirmation_score)
        false_positive_ceiling = float(self.settings.screener_max_false_positive_risk)
        if timeframe == "1m":
            ...
            accuracy_floor = max(accuracy_floor, float(self.settings.screener_min_accuracy_score) + 0.08)
            confirmation_floor = max(confirmation_floor, float(self.settings.screener_min_confirmation_score) + 0.10)
            false_positive_ceiling = min(false_positive_ceiling, float(self.settings.screener_max_false_positive_risk) - 0.08)
        ...
        self._check(
            confirmation_score >= confirmation_floor,
            reasons,
            rejections,
            "confirmation_ok",
        ...
        self._check(
            false_positive_risk <= false_positive_ceiling,
            reasons,
            rejections,
            "false_positive_risk_ok",
        ...
        accuracy_measurements = {
            ...
            for key in [
                ...
                "nearest_anchor_distance_atr",
                "nearest_barrier_distance_atr",
                "entry_extension_atr",
            ]
        ...
        if intelligence is not None:
            ...
            measurements = {**context.measurements, **accuracy_measurements}
