...
class EToroClient(BrokerClient):
    ...
    def get_portfolio(self) -> PortfolioSummary:
        ...
        # The /pnl endpoint returns 404 on many real-mode accounts; the portfolio
        # response already carries credit + position-level data we need. Treat pnl
        # as a best-effort enrichment, not a hard dependency.
        pnl: dict[str, Any] = {}
        try:
            pnl_payload = self._request("GET", self._trading_info_path("pnl"))
            pnl = pnl_payload.get("clientPortfolio", {}) or {}
        except RuntimeError as exc:
            logger.info("pnl endpoint unavailable, deriving from portfolio: %s", exc)
    ...
    def fetch_raw_portfolio(self) -> dict[str, Any]:
        """Return the raw /trading/info/portfolio payload.

        Exposes fields the normalized ``PortfolioSummary`` doesn't keep
        (stopLossRate, takeProfitRate, openDateTime, totalFees, etc.) so
        the outcome ledger can reconstruct position lifecycles.
        """
        if self.settings.broker_simulation_enabled:
        ...
        *,
        ...
        try:
            ...
            if mark_etoro_rate_limited(
                ...
                logger.warning("eToro broker request rate-limited: %s %s", exc, body)
    ...
    def _trading_execution_path(self, suffix: str) -> str:
        # Verified 2026-04-19 via scripts/verify_etoro_demo.py: the /demo/ execution
        # prefix returned 403 InsufficientPermissions on a real-funded eToro account.
        # Real-mode is the only mode this key can place orders in. If you later obtain
        # a separately provisioned demo key, branch here on account_mode.
        return f"/trading/execution/{suffix}"

    def _trading_info_path(self, suffix: str) -> str:
        # Verified 2026-04-19 via scripts/verify_etoro_demo.py: /trading/info/demo/*
        # returns 403 InsufficientPermissions; /trading/info/portfolio returns 200.
        # /trading/info/pnl returns 404 — callers should treat pnl as best-effort.
