...
def test_strategy_catalog_reports_core_enhanced_and_governance_counts(tmp_path):
    ...
    assert report["total_strategy_families"] == 18
    assert report["total_strategy_specs"] == 50
    assert report["total_active_specs"] == 50
    assert report["core_strategy_families"] == 12
    assert report["core_strategy_specs"] == 36
    assert report["enhanced_research_strategy_families"] == 6
    assert report["enhanced_research_strategy_specs"] == 14
...
def test_scheduled_all_buckets_cover_core_and_enhanced_specs():
    ...
    assert len(covered_specs) == 50
    assert len([spec for spec in covered_specs if spec.name in CORE_STRATEGY_NAMES]) == 36
    assert len([spec for spec in covered_specs if spec.name in ENHANCED_RESEARCH_STRATEGY_NAMES]) == 14
