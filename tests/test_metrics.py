...
def test_metrics_exposes_reconciliation_trading_and_safety_gauges(tmp_path):
    ...
    assert "algobot_strategy_families_total 28" in response.text
    assert "algobot_strategy_specs_total 70" in response.text
    assert "algobot_strategy_families_enhanced_total 16" in response.text
    assert "algobot_strategy_specs_enhanced_total 34" in response.text
