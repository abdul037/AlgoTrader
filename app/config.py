...
class AppSettings(BaseSettings):
    ...
    def database_path(self) -> Path:
        """Return the SQLite file path for the configured database URL."""

        prefix = "sqlite:///"
        if not self.database_url.startswith(prefix):
            raise ValueError(
                "Only sqlite:/// URLs are supported in this version. "
                "Keep the repository abstraction to add PostgreSQL later."
            )
    ...
    def real_mode_requested(self) -> bool:
        """Return whether the configured account mode is real."""

        return self.etoro_account_mode == "real"

    @property
    def broker_simulation_enabled(self) -> bool:
        """Use safe simulated broker responses when credentials/endpoints are not configured."""
    ...
    def telegram_mode(self) -> str:
        """Return the active Telegram delivery mode."""
