...
class MarketScreenerService:
    ...
    def scan_universe(
        ...
        *,
        ...
        for symbol in universe:
            for timeframe in scan_timeframes:
                ...
                for spec in get_strategy_specs(timeframe=timeframe):
                    ...
                    backtest = self._backtest_validation(signal.symbol, signal.strategy_name)
        ...
        response = ScreenerRunResponse(
            ...
            universe_name=self.settings.market_universe_name,
            timeframes=scan_timeframes,
            evaluated_symbols=len(universe),
    ...
    def _snapshot_from_signal(
        ...
        *,
        ...
        return LiveSignalSnapshot(
            ...
            reject_reasons=[],
            indicators={**context.measurements, **intelligence.measurements},
            metadata=metadata,
            backtest_snapshot=backtest_snapshot,
        )

    def _backtest_validation(self, symbol: str, strategy_name: str) -> dict[str, Any]:
        if self.backtests is None:
            return {"passes": False, "reason": "no_backtest_repository", "summary": None}
        summary = self.backtests.get_latest_summary(symbol.upper(), strategy_name)
        if summary is None:
    ...
    def _build_no_trade_snapshot(
        ...
        *,
        ...
        market_data_status = self._market_data_status(history=history, quote=quote)
        recent_decisions = self.scan_decisions.list(limit=10, symbol=symbol) if self.scan_decisions is not None else []
        ...
        for item in recent_decisions:
            ...
            if reasons:
                ...
                top_score = float(item.final_score or 0.0)
                break
        ...
        metadata = {
            ...
            "trade_plan": {
                ...
                "confirmation_trigger": "Wait for a stronger regime-aligned setup with clearer relative strength.",
                "target_1": None,
                "target_2": None,
                "stretch_target": None,
        ...
        return LiveSignalSnapshot(
            ...
            freshness="fresh",
            rationale=rationale,
            score=max(min(top_score, 54.0), 35.0) if top_score else 42.0,
            score_breakdown={"market_regime": round(intelligence.market_regime_score * 100.0, 2)},
            confidence=0.25,
            tradable=False,
            execution_ready=False,
            ...
            indicators={**intelligence.measurements},
            metadata=metadata,
            backtest_snapshot={},
        )
